...
int main()
{
    ...
    start_DP = read_QPC(); 
    koszt_minimalny_wyswietl = algorytmDP(1 << 0, 0, ilosc_miast, macierz_sasiedztwa);
    elapsed_DP = read_QPC() - start_DP; 

    cout << " Koszt minimalny: " << koszt_minimalny_wyswietl << endl;

    if (ilosc_miast > 12)
    {
        cout << "-------------------------------------------------" << endl;
        cout << " Liczba miast zbyt duza by wykonac przeglad zupelny!";
        elapsed_BF = 0;
    }
    else
    {
